...
def get_city_lat_lng(city_name: str | None, state_name: str | None, country_name: str, lat_lng_report_file: TextIOWrapper | None):
    if(city_name is None):
        return get_state_lat_lng(state_name, country_name, lat_lng_report_file)
    
    mapbox_response = make_mapbox_request(city_name, state_name, country_name, lat_lng_report_file, None)
...
def get_state_lat_lng(state_name: str | None, country_name: str, lat_lng_report_file: TextIOWrapper | None):
    if(state_name is None):
        return get_country_lat_lng(country_name, lat_lng_report_file)
    
    mapbox_response = make_mapbox_request(None, state_name, country_name, lat_lng_report_file, None)
    if(mapbox_response == None):
        return get_country_lat_lng(country_name, lat_lng_report_file)
    coords = mapbox_response.center_coordinates
    if(coords is None):
...
def get_country_lat_lng(country_name: str, lat_lng_report_file: TextIOWrapper | None):
    return make_mapbox_request(None, None, country_name, lat_lng_report_file, None).center_coordinates
